...
def calculate_and_hide():
    ...
    # If the current time is before 5:29 AM, set elapsedMinutes to 0
    ...
    # Format the current period
    formatted_date = now.strftime('%Y%m%d')
    period_string = "100001" + f"{elapsed_minutes:04d}"
    current_period = formatted_date + period_string

    print(f"{BRIGHT_BLUE}═════════════════════════════════════════════════════════════{RESET}")
    print(f"{BRIGHT_WHITE}Current Period: {BRIGHT_CYAN}{current_period}{RESET}")  # Current period on top
    print(f"{BRIGHT_GREEN}Accuracy: {accuracy}%{RESET}")
    print(f"{BRIGHT_GREEN}Final Result: {result_message}{RESET}")  # Show final result message
    print(f"{BRIGHT_GREEN}FPS: {fps}{RESET}")  # Show FPS here
    print(f"{BRIGHT_BLUE}═════════════════════════════════════════════════════════════{RESET}")

    # Append the results to the history
    result_history.append(f"Period: {current_period} | Result: {result_message} | Accuracy: {accuracy}% | FPS: {fps}")
